...
**Production `sync_runs` has three rows that aren't real CI runs** and nobody should read them as history: id 1 is hand-inserted test data (start and finish share an identical millisecond fraction, exactly 6000ms apart); id 7 is a local `npm run sync:votes` against production from a laptop; ids 8 and 9 were written accidentally by a QA harness whose module mock resolved to the wrong path. All are inert — later real rows supersede them, and no data table was touched. Deleting them is a production write and the owner's call.
