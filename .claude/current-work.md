...
- [x] 5. Bill summaries **(done 2026-09-25)** — spec: `.claude/specs/slice-05-bill-summaries.md` — `bills.summary` populated from Congress.gov for 456 of 509 bills and rendered on `/votes/[id]`, clamped with an expand toggle. **Both carried-over notes resolved:** the `bill_title ?? description ?? question` chain is untouched, because `short_title` stays NULL by decision (measured: it would improve roughly three of 509 bills, and Congress.gov publishes none for the resolution types whose titles are actually long) — the trigger and the two sites that must change together are now recorded in a comment on `leadTextFor`; and `truncate` gained its `max <= 0` guard with a live caller (`/votes/[id]`'s meta description passes `META_DESCRIPTION_MAX - prefix.length`) plus boundary tests.
- [ ] 6. Real search — **do not trigram-index `bills.summary` raw**: a 168,260-char value under `gin_trgm_ops` builds an enormous posting list and `similarity()` against a short query is ~0, so that bill becomes *less* findable. Index a bounded prefix or a `tsvector`. — install `pg_trgm`, index members and bills, fuzzy + ranked; extend search to bills; correct the docs.
- [ ] 7. Cache against the nightly sync — **also cap the summary text served to the browser**: it is passed as a prop to a client component, so it serializes twice (SSR HTML + RSC flight payload). `/votes/house-119-1-190` measures 488 KB raw / 113 KB gzipped against 86 KB / 13.7 KB for a page without a large summary — 8× the bytes to render six visible lines. 73 roll calls join a bill whose summary exceeds 50,000 chars. — also truncate description text in the `/bills` query (~200 KB of nominee lists now flow into a page that renders all 1,573 cards). Drop `force-dynamic` from `/`, `/members`, `/bills` and the party pages; revalidate on ingest instead of per request, so the DB is hit on a cache miss rather than on a pageview. Last in Phase A so we aren't caching data we're still fixing.
...
Slices 1–5 merged; the cron is live and its first scheduled run succeeded. **Next: slice 6 (real search).** Then slice 7 (caching) closes Phase A, and Phase B begins with the design direction spike.
...
- **The expand/collapse clipping detection latches one way.** Once it measures "not clipped" it removes the clamp class, so every later measurement is taken on an unclamped element and can only ever confirm "not clipped". Narrowing the window never re-clamps: `/votes/house-119-1-58` at 1280px then resized to 320px renders 18 lines with no toggle. The cheap fix (keep the clamp whenever collapsed, use the measurement only to decide whether to render the button) costs progressive enhancement — a short summary would render clamped with no reachable toggle before hydration. That tradeoff is a Phase B decision. → slice 12.
- **A failed bill-detail fetch still fabricates `title`, `status` and `latest_action_date`** rather than leaving them alone — the placeholder `HR 1234` overwrites a real stored title. Slice 5 added a retry (taking the per-bill probability from *p* to *p²*) and an annotation, but the bare `excluded.*` writes are pinned by its criterion B, so changing them is a spec amendment. This is the last instance of the nulling class that `summary` got `coalesce` against and slice 3's `published_*` got fixed for — and the only one on a `NOT NULL` display column.
- **Bill-detail failures reach the annotation list but not the step-summary table**, and GitHub renders at most 10 annotations per level per step — so a full Congress.gov outage on a ~60-bill night shows 10 visible annotations and a step summary that looks clean. The log has all of them. Spec amendment, not a defect.
...
From slice 5's review.

- **The four signal paths in `scripts/sync-votes.ts` remain untested**, and slice 5's `lib/bill-summary.ts` extraction does not change that — it adds one more tested pure module and covers none of them. A `scripts/` test harness needs a non-production database; the first one is the Neon branch reserved for **slice 13**, and a previous attempt without one wrote rows 8 and 9 into production `sync_runs`. → slice 13, not before.
