...
- **Ingest verifies itself.** Both chambers publish an official tally (`<yeas>54</yeas>`) next to the per-member positions. Stored positions must sum to it or the sync fails loudly. This is what would have caught the ~3,300 positions currently dropped in silence.
...
- [ ] 14. Historical roster — ingest `legislators-historical`, one term row per person-per-Congress; recovers the ~3,300 dropped positions.
...
- `docs/expansion/` is still untracked. Commit it.
